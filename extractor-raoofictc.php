<?php
/**
 * اکسترکتور محصولات برای فروشگاه raoofictc.com (اسپرت خودرو رئوفی).
 *
 * سایت مبدأ یک فروشگاه استاندارد WooCommerce با قالب Woodmart است. صفحه‌ی
 * محصول به‌طور کامل در سمت سرور رندر می‌شود، از جمله برای محصولات متغیر:
 * فرم `form.variations_form` همان لحظه‌ی اول یک ویژگی `data-product_variations`
 * دارد که JSON کامل تمام واریانت‌ها (قیمت، موجودی، تصویر، SKU هر واریانت) را
 * در خودش حمل می‌کند؛ یعنی این داده از همان اولین درخواست HTTP (بدون اجرای
 * جاوااسکریپت) در دسترس است.
 *
 * نکته‌ی مهم دربارهٔ «دیر لود شدن واریانت‌ها»: چیزی که در مرورگر دیر ظاهر
 * می‌شود، فقط رابط بصری انتخاب‌گر سواچ (دکمه‌های رنگی/شکلی پایه‌ی هدلایت)
 * است که با جاوااسکریپت و بعد از اجرای افزونه‌ی سواچ‌های ووکامرس روی صفحه
 * نقش می‌بندد؛ خودِ HTML خام (که این اکسترکتور با wp_safe_remote_get می‌خواند)
 * از قبل کامل است و نیازی به صبر کردن یا اجرای مرورگر Headless ندارد. با
 * این‌همه، به‌خاطر لایه‌های کش/لِیزی‌لود این سایت (Seraphinite Accelerator)،
 * احتمال کمی وجود دارد که یک نسخه‌ی کش‌شدهٔ ناقص برگردد؛ برای اطمینان،
 * extract_product_data() اگر محصول را «متغیر» تشخیص دهد ولی هیچ واریانتی
 * از data-product_variations استخراج نشود، یک بار با یک درخواست تازه
 * (بدون کش مرورگر/سرور با کوئری‌استرینگ cache-busting) دوباره تلاش می‌کند.
 *
...
class Raoofictc_Product_Extractor {
	...
	const MENU_SLUG = 'raoofictc-product-extractor';
	const NONCE_ACTION = 'raoofictc_extractor_action';
	...
	public function render_admin_page() {
		...
		if ( isset( $_POST['raoofictc_extractor_submit'] ) ) {
			check_admin_referer( self::NONCE_ACTION );

			$url = isset( $_POST['raoofictc_url'] ) ? esc_url_raw( wp_unslash( $_POST['raoofictc_url'] ) ) : '';
		...
		?>
		<div class="wrap">
			<h1>اکسترکتور محصولات راوفی (RaoofICTC)</h1>
			<form method="post">
				<?php wp_nonce_field( self::NONCE_ACTION ); ?>
				<table class="form-table">
					<tr>
						<th scope="row"><label for="raoofictc_url">آدرس محصول</label></th>
						<td>
							<input type="url" id="raoofictc_url" name="raoofictc_url" class="regular-text" required
								value="<?php echo esc_attr( $url ); ?>" placeholder="https://raoofictc.com/product/..." />
						</td>
					</tr>
				</table>
				<p class="submit">
					<input type="submit" name="raoofictc_extractor_submit" class="button button-primary" value="استخراج اطلاعات" />
				</p>
			</form>
	...
	/**
	 * تشخیص این‌که آیا یک HTML از قبل واکشی‌شده متعلق به فروشگاه raoofictc.com
	 * است یا نه. نیازی به دسترسی شبکه ندارد.
	 *
	 * @param string $html HTML از قبل واکشی‌شدهٔ صفحه.
	 * @return bool
	 */
	public static function is_platform_match( $html ) {
		...
		if ( false !== stripos( $html, 'raoofictc.com' ) ) {
			return true;
		}
		return false;
	}
	...
	/**
	 * کشف آدرس محصولات از طریق sitemap سایت (اولویت با sitemap اختصاصی
	 * ووکامرس/RankMath که در raoofictc.com فعال است). این روش کاملاً
	 * ایستا و سمت سرور است و به هیچ اجرای جاوااسکریپتی وابسته نیست، پس
	 * مشکل «دیر ظاهر شدن سواچ‌های واریانت» در مرورگر روی آن اثری ندارد.
	 *
	 * @param array $profile پروفایل منبع؛ باید url/base_url یا sitemap_url داشته باشد.
	...
	public static function get_product_urls( $profile ) {
		...
		if ( '' === $sitemap_url ) {
			$origin      = '' !== $site_url ? $site_url : 'https://raoofictc.com/';
			$parts       = wp_parse_url( $origin );
			$origin      = ( isset( $parts['scheme'] ) ? $parts['scheme'] : 'https' ) . '://' . ( isset( $parts['host'] ) ? $parts['host'] : 'raoofictc.com' );
			$sitemap_url = $origin . '/sitemap_index.xml';
		}
		...
		foreach ( $urls as $u ) {
			...
			if ( is_string( $path ) && preg_match( '#^/product/#', $path ) ) {
				$filtered[] = $u;
			}
		}

		if ( empty( $filtered ) ) {
			return new WP_Error( 'raoofictc_no_products', 'هیچ آدرس محصولی از sitemap پیدا نشد.' );
		}
	...
	private function collect_sitemap_urls( $sitemap_url, $depth ) {
		...
		$response = wp_safe_remote_get(
			...
			array(
				...
				'headers'             => array( 'User-Agent' => 'Mozilla/5.0 (compatible; Raoofictc-Extractor/1.0; +wordpress)' ),
			)
		);
	...
	public function extract_product_data( $url ) {
		...
		$result = $this->parse_product_html( $html, $url );

		// اگر محصول «متغیر» تشخیص داده شد ولی به هر دلیلی (کش قدیمی/ناقص)
		// هیچ واریانتی از data-product_variations استخراج نشد، یک بار دیگر
		// با پرهیز صریح از کش تلاش می‌کنیم تا محصول به‌اشتباه «ساده» ثبت نشود.
		$product_type = is_array( $result ) ? (string) $this->array_get( $result, 'product_type', '' ) : '';
		$variations   = is_array( $result ) ? $this->array_get( $result, 'variations', array() ) : array();
		if ( 'variable' === $product_type && empty( $variations ) ) {
			$fresh_response = $this->fetch_product_page( $url, true );
			if ( ! is_wp_error( $fresh_response ) ) {
				$fresh_status = (int) wp_remote_retrieve_response_code( $fresh_response );
				if ( $fresh_status >= 200 && $fresh_status < 300 ) {
					$fresh_html = wp_remote_retrieve_body( $fresh_response );
					if ( ! empty( $fresh_html ) && is_string( $fresh_html ) ) {
						$fresh_result = $this->parse_product_html( $fresh_html, $url );
						$fresh_variations = is_array( $fresh_result ) ? $this->array_get( $fresh_result, 'variations', array() ) : array();
						if ( ! empty( $fresh_variations ) ) {
							$result = $fresh_result;
						}
					}
				}
	...
	private function validate_product_url( $url ) {
		...
		if ( ! in_array( $host, array( 'raoofictc.com', 'www.raoofictc.com' ), true ) ) {
			return 'آدرس باید متعلق به دامنه raoofictc.com باشد.';
		}
	...
	private function fetch_product_page( $url, $bypass_cache = false ) {
		$current = $url;
		if ( $bypass_cache ) {
			$current = add_query_arg( array( 'mss_nocache' => (string) time() ), $current );
		}
		for ( $i = 0; $i < 5; $i++ ) {
			...
			$args = array(
				...
				'user-agent' => 'Mozilla/5.0 (compatible; Raoofictc-Extractor/1.0; +wordpress)',
			);
			if ( $bypass_cache ) {
				$args['headers'] = array( 'Cache-Control' => 'no-cache, no-store', 'Pragma' => 'no-cache' );
			}
	...
	private function extract_from_dom( $html, $base_url ) {
		...
		// توضیحات کوتاه: باید ساختار HTML (مثل <ul><li>) حفظ شود، وگرنه همه‌ی سطرها
		// در یک خط پلین به‌هم می‌چسبند. برای همین به‌جای node_text از inner_html استفاده می‌شود.
		$short_description = '';
		...
		if ( $short_desc_node ) {
			$this->fix_lazy_images( $xpath, $short_desc_node );
			$short_description = trim( $this->inner_html( $short_desc_node ) );
		}

		// توضیحات اصلی محصول. در raoofictc.com (و مشابه آن) توضیحات اصلی داخل قالب
		// کلاسیک ووکامرس (#tab-description) نیست، بلکه در اولین تب ویجت Elementor
		// Nested Tabs (کلاس e-n-tabs-content، پنل با data-tab-index="1") قرار دارد.
		// هر دو الگو را امتحان می‌کنیم تا اگر ساختار قالب در برخی صفحات فرق داشت هم کار کند.
		$content_html = '';
		$desc_node    = $this->query_one( $xpath, ".//*[@id='tab-description']", $doc->documentElement );
		if ( $desc_node ) {
			$this->fix_lazy_images( $xpath, $desc_node );
			$content_html = $this->inner_html( $desc_node );
		}
		if ( '' === trim( wp_strip_all_tags( $content_html ) ) ) {
			$tabs_wrap = $this->query_one( $xpath, ".//*[contains(concat(' ', normalize-space(@class), ' '), ' e-n-tabs-content ')]", $doc->documentElement );
			if ( $tabs_wrap ) {
				$panel = $this->query_one( $xpath, "./*[@data-tab-index='1']", $tabs_wrap );
				if ( $panel ) {
					$parts        = array();
					$editor_nodes = $xpath->query( ".//*[contains(concat(' ', normalize-space(@class), ' '), ' elementor-widget-text-editor ')]//*[contains(concat(' ', normalize-space(@class), ' '), ' elementor-widget-container ')]", $panel );
					if ( $editor_nodes && $editor_nodes->length > 0 ) {
						foreach ( $editor_nodes as $en ) {
							$this->fix_lazy_images( $xpath, $en );
							$html_part = trim( $this->inner_html( $en ) );
							if ( '' !== $html_part ) {
								$parts[] = $html_part;
							}
						}
					} else {
						$this->fix_lazy_images( $xpath, $panel );
						$part = trim( $this->inner_html( $panel ) );
						if ( '' !== $part ) {
							$parts[] = $part;
						}
					}
	...
	/**
	 * تصحیح تصاویر lazy-load شده (که src آن‌ها یک placeholder کوچک base64 است)
	 * درون یک بخش از محتوا (توضیحات کوتاه/اصلی)، و حذف تگ‌های script/style/noscript
	 * تا در خروجی نهایی، تصویر واقعی نمایش داده شود نه یک تصویر شکسته/جایگزین.
	 * این متد مستقیماً روی DOM عمل می‌کند (قبل از serialize شدن به HTML).
	 *
	 * @param DOMXPath $xpath
	 * @param DOMNode  $context
	 */
	private function fix_lazy_images( $xpath, $context ) {
		if ( ! $context ) {
			return;
		}

		$imgs = $xpath->query( './/img', $context );
		if ( $imgs ) {
			foreach ( $imgs as $img ) {
				$real = '';
				foreach ( array( 'data-lzl-src', 'data-src', 'data-lazy-src', 'data-large_image' ) as $attr ) {
				...
				if ( '' === $real ) {
					// آخرین راه‌چاره: تصویر واقعی معمولاً داخل <noscript> مجاور هم قرار دارد
					// (چون libxml محتوای noscript را به‌صورت متن خام نگه می‌دارد، نه DOM واقعی)
					$noscript = $xpath->query( './following-sibling::noscript[1]', $img );
					if ( $noscript && $noscript->length > 0 ) {
						$raw = $noscript->item( 0 )->textContent;
						if ( $raw && preg_match( '/<img[^>]+src=["\']([^"\']+)["\']/i', $raw, $m ) ) {
							$real = $m[1];
						}
					}
				}
				$current_src = trim( $img->getAttribute( 'src' ) );
				if ( '' !== $real && ( '' === $current_src || 0 === strpos( $current_src, 'data:' ) ) ) {
					$img->setAttribute( 'src', $real );
				}
				foreach ( array( 'data-lzl-src', 'data-src', 'data-lazy-src', 'data-large_image', 'srcset', 'data-srcset', 'loading' ) as $attr ) {
	...
	private function normalize_extracted_data( $raw, $base_url ) {
		...
		if ( '' === trim( wp_strip_all_tags( $content_html ) ) && '' !== trim( wp_strip_all_tags( $excerpt ) ) ) {
			// excerpt از قبل ممکن است خودش HTML باشد (مثلاً <ul><li>...)؛ در این حالت
			// دیگر نباید دوباره با esc_html() اسکیپ شود وگرنه تگ‌ها به‌صورت متن نمایش داده می‌شوند.
			$content_html = ( $excerpt !== wp_strip_all_tags( $excerpt ) ) ? $excerpt : ( '<p>' . esc_html( $excerpt ) . '</p>' );
		}
