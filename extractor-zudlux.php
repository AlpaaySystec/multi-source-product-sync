...
class Zudlux_Product_Extractor {
	...
	const MENU_SLUG = 'zudlux-product-extractor';
	const NONCE_ACTION = 'zudlux_extractor_action';
	...
	public function render_admin_page() {
		...
		if ( isset( $_POST['zudlux_extractor_submit'] ) ) {
			check_admin_referer( self::NONCE_ACTION );

			$url = isset( $_POST['zudlux_url'] ) ? esc_url_raw( wp_unslash( $_POST['zudlux_url'] ) ) : '';
		...
		?>
		<div class="wrap">
			<h1>Zudlux Product Extractor</h1>
			<form method="post">
				<?php wp_nonce_field( self::NONCE_ACTION ); ?>
				<table class="form-table">
					<tr>
						<th scope="row"><label for="zudlux_url">آدرس محصول</label></th>
						<td>
							<input type="url" id="zudlux_url" name="zudlux_url" class="regular-text" required
								value="<?php echo esc_attr( $url ); ?>" placeholder="https://zudlux.com/product/..." />
						</td>
					</tr>
				</table>
				<p class="submit">
					<input type="submit" name="zudlux_extractor_submit" class="button button-primary" value="استخراج اطلاعات" />
				</p>
			</form>
	...
	private function validate_product_url( $url ) {
		...
		if ( ! in_array( $host, array( 'zudlux.com', 'www.zudlux.com' ), true ) ) {
			return 'آدرس باید متعلق به دامنه zudlux.com باشد.';
		}
	...
	private function fetch_product_page( $url ) {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			...
			$response = wp_safe_remote_get( $current, array(
				...
				'user-agent' => 'Mozilla/5.0 (compatible; Zudlux-Extractor/2.0; +wordpress)',
			) );
	...
	private function extract_source_data( $html, $url, $standard_raw ) {
		...
		$gtm4wp_product_data = array();
		foreach ( $xpath->query( "//input[@name='gtm4wp_product_data' and @value] | //*[@data-gtm4wp_product_data]" ) as $gtm_node ) {
			$encoded = $gtm_node->hasAttribute( 'value' ) ? $gtm_node->getAttribute( 'value' ) : $gtm_node->getAttribute( 'data-gtm4wp_product_data' );
			$decoded = json_decode( html_entity_decode( $encoded, ENT_QUOTES, 'UTF-8' ), true );
			if ( is_array( $decoded ) ) {
				$gtm4wp_product_data[] = $decoded;
			}
		}
	...
	private function display_source_data_sections( $source_data ) {
		...
		$sections = array(
			...
			'دادهٔ تجارت الکترونیک GTM4WP' => isset( $source_data['gtm4wp_product_data'] ) ? $source_data['gtm4wp_product_data'] : array(),
